...
class Tracker:
    ...
    # def update(self, detections): #the original update (DO NOT DELETE)
    ...
    #         entering, fraction = is_entering_store_percent(bbox, line_y = 108, threshold=0.60) #there is a slim change that a person leaving the store might detect this function if the detection of the person leaving the store is unmatched and >= 0.7 of the box is below the green line
    #         if entering:
    ...
    #     """
    #     Modified update method that first checks for overlapping detections.
    #     For detections whose bounding boxes overlap above a given threshold,
    #     the global database is directly queried for a match—unless the detection
    #     is near the entrance (in which case a new ID is forced).
    #     """
    #     overlap_threshold = 0.25  # IoU threshold for considering boxes as overlapping
    #     num_dets = len(detections)
    #     # Dictionary to hold indices of detections that are pre-matched via global database.
    #     pre_matched = {}

    #     # First pass: For every unique pair, check overlap.
    #     for i in range(num_dets):
    #         box_i = detections[i].to_tlbr()
    #         # Check if detection i is near the entrance.
    #         entering_i, frac_i = is_entering_store_percent(box_i, line_y=108, threshold=0.70)
    #         for j in range(i + 1, num_dets):
    #             box_j = detections[j].to_tlbr()
    #             entering_j, frac_j = is_entering_store_percent(box_j, line_y=108, threshold=0.70)
    #             iou = compute_iou(box_i, box_j)
    #             logger.info(f"overlap between {box_i} and {box_j} is {iou}")
    ...
    #                 logger.info(f"Significant overlap: fraction_A={fraction_A:.2f}, fraction_B={fraction_B:.2f} for boxes {box_i} and {box_j}")
    #                 logger.info(f"Overlap of {iou:.2f} for boxes {box_i} and {box_j}")
    #                 # For detection i: only pre-match if it is not near the entrance.
    #                 if i not in pre_matched and not entering_i:
    #                     logger.info(f"Matching {box_i} with global database features")
    #                     track_id = self._match_with_global_database_all_tracks_considered(detections[i].feature, box_i)
    #                     if track_id is not None:
    #                         pre_matched[i] = track_id
    #                 # Similarly for detection j.
    #                 if j not in pre_matched and not entering_j:
    #                     logger.info(f"Matching {box_j} with global database features")
    #                     logger.info(f"Matching {box_i} with global database features")
    #                     track_id = self._match_with_global_database_all_tracks_considered(detections[j].feature, box_j)
    #                     if track_id is not None:
    #                         pre_matched[j] = track_id

    #     # Now, separate detections that will be processed by the matching cascade.
    #     cascade_indices = [idx for idx in range(num_dets) if idx not in pre_matched]
    #     cascade_detections = [detections[idx] for idx in cascade_indices]

    #     # Run the original matching cascade on the remaining detections.
    #     matches_cascade, unmatched_tracks, unmatched_dets_cascade = self._match(cascade_detections)

    #     # Update tracks for the cascade matches.
    #     for track_idx, cascade_det_idx in matches_cascade:
    #         # Map back to the original detection if needed.
    #         self.tracks[track_idx].update(self.kf, cascade_detections[cascade_det_idx], self.global_database)
    #         logger.info(f"Matched cascade detection to track_id {self.tracks[track_idx].track_id}.")

    #     # Process unmatched cascade detections.
    #     for cascade_det_idx in unmatched_dets_cascade:
    #         detection = cascade_detections[cascade_det_idx]
    #         bbox = detection.to_tlbr()
    #         # Check if this detection is near the entrance.
    #         entering, fraction = is_entering_store_percent(bbox, line_y=108, threshold=0.70)
    #         if entering:
    #             logger.info(f"Detection with bbox {bbox} is entering the store (fraction below line: {fraction}).")
    #             matched_track_id = None  # Force new ID assignment.
    #         else:
    #             matched_track_id = self._match_with_global_database(detection.feature, bbox)
    #         if matched_track_id is not None:
    #             # Re-identify the track using the database.
    #             mean, covariance = self.kf.initiate(detection.to_xyah())
    #             class_name = detection.get_class()
    #             self.tracks.append(Track(mean, covariance, matched_track_id, self.n_init, self.max_age,
    #                                     detection.feature, class_name))
    #             # Optionally, copy features from the global database.
    #             self.tracks[-1].features = self.global_database[matched_track_id]["features"]
    #             logger.info(f"Re-identified detection with global match: track_id {matched_track_id}.")
    #         else:
    ...
    #                                     detection.feature, class_name))
    #             logger.info(f"Assigned new track_id {self._next_id} for unmatched detection.")
    #             self._next_id += 1

    #     # Now process the detections that were pre-matched (overlapping ones).
    #     for idx, track_id in pre_matched.items():
    #         detection = detections[idx]
    #         mean, covariance = self.kf.initiate(detection.to_xyah())
    #         class_name = detection.get_class()
    #         self.tracks.append(Track(mean, covariance, track_id, self.n_init, self.max_age,
    #                                 detection.feature, class_name))
    #         logger.info(f"Pre-matched overlapping detection assigned track_id {track_id}.")

    #     # Mark unmatched tracks from the cascade as missed.
    #     for track_idx in unmatched_tracks:
    #         self.tracks[track_idx].mark_missed()
    ...
    #     # Finally, update the distance metric.
    #     active_targets = [t.track_id for t in self.tracks if t.is_confirmed()]
    #     features, targets = [], []
    #     for track in self.tracks:
    #         if not track.is_confirmed():
    ...
    # def update(self, detections): #includes logic to combat switches while overlap
    #     """
    #     Modified update method that first checks for overlapping detections.
    #     For detections whose bounding boxes overlap above a given threshold,
    #     the global database is directly queried for a match.
    #     """
    #     overlap_threshold = 0.25 # Define your IoU threshold for "overlap"
    #     num_dets = len(detections)
    #     # Dictionary to hold indices of detections that are pre-matched via global database:
    #     pre_matched = {}

    #     # First pass: Check each unique pair for high IoU.
    #     for i in range(num_dets):
    #         box_i = detections[i].to_tlbr()
    #         for j in range(i + 1, num_dets):
    #             box_j = detections[j].to_tlbr()
    #             iou = compute_iou(box_i, box_j)
    #             if iou > overlap_threshold:
    #                 logger.info(f"Overlap of {iou} for {box_i} and {box_j}")
    #                 # For detection i, if not already pre-matched, try global database match.
    #                 if i not in pre_matched:
    #                     track_id = self._match_with_global_database_all_tracks_considered(detections[i].feature, box_i)
    #                     if track_id is not None:
    #                         pre_matched[i] = track_id
    #                 # Similarly for detection j.
    #                 if j not in pre_matched:
    #                     track_id = self._match_with_global_database_all_tracks_considered(detections[j].feature, box_j)
    #                     if track_id is not None:
    #                         pre_matched[j] = track_id

    #     # Now, separate detections that will be handled by the matching cascade.
    #     cascade_indices = [idx for idx in range(num_dets) if idx not in pre_matched]
    #     cascade_detections = [detections[idx] for idx in cascade_indices]
    ...
    #     # Update tracks for the cascade matches.
    #     for track_idx, cascade_det_idx in matches_cascade:
    #         # Map back to the original detection index if needed.
    #         self.tracks[track_idx].update(self.kf, cascade_detections[cascade_det_idx], self.global_database)
    #         logger.info(f"Matched cascade detection to track_id {self.tracks[track_idx].track_id}.")
    ...
    #     for cascade_det_idx in unmatched_dets_cascade:
    #         detection = cascade_detections[cascade_det_idx]
    #         track_id = self._match_with_global_database(detection.feature, detection.to_tlbr())
    #         if track_id is not None:
    #             # Re-identify using global database match.
    #             mean, covariance = self.kf.initiate(detection.to_xyah())
    #             class_name = detection.get_class()
    #             self.tracks.append(Track(mean, covariance, track_id, self.n_init, self.max_age, detection.feature, class_name))
    #         else:
    #             # Assign a new track id.
    #             mean, covariance = self.kf.initiate(detection.to_xyah())
    #             class_name = detection.get_class()
    ...
    #     for idx, track_id in pre_matched.items():
    #         detection = detections[idx]
    #         # Before creating a new track, you might check if this track is already active.
    #         # Here we simply create a new track with the matched global database id.
    #         mean, covariance = self.kf.initiate(detection.to_xyah())
    #         class_name = detection.get_class()
    #         self.tracks.append(Track(mean, covariance, track_id, self.n_init, self.max_age, detection.feature, class_name))
    #         logger.info(f"Pre-matched overlapping detection assigned track_id {track_id}.")
    ...
    #     # Finally, remove any deleted tracks.
    #     self.tracks = [t for t in self.tracks if not t.is_deleted()]

    #     # Update the distance metric based on features from confirmed tracks.
    #     active_targets = [t.track_id for t in self.tracks if t.is_confirmed()]
    #     features, targets = [], []
    #     for track in self.tracks:
    #         if not track.is_confirmed():
    ...
    #     Includes an exception for the first 125 frames (5 seconds) to handle abrupt camera startup.
    ...
    #     EXCEPT during the first 125 frames where new IDs are allowed anywhere.
    #     """
    #     # Step 1: Identify potentially overlapping detections
    #     overlap_threshold = 0.25  # IoU threshold
    #     num_dets = len(detections)
    ...
    #     # Check if we're in the startup grace period (first 5 seconds / 125 frames)
    #     startup_grace_period = self._frame_count < 251
    #     if startup_grace_period:
    #         logger.info(f"In startup grace period (frame {self._frame_count}/125): allowing new IDs anywhere in frame")
    ...
    #     # First process the good matches from the cascade directly
    #     # This is key for maintaining track consistency
    #     for track_idx, detection_idx in matches:
    #         track = track_idx_to_track[track_idx]
    #         detection = detections[detection_idx]
    ...
    #     # Create sets to track assigned IDs and detections
    #     assigned_ids = set()
    #     assigned_detections = set()
    #     for track_idx, detection_idx in matches:
    #         assigned_ids.add(track_idx_to_track[track_idx].track_id)
    #         assigned_detections.add(detection_idx)
        
    #     # Step 3: Categorize remaining detections
    #     entering_detections = set()  # Detections determined to be entering
    ...
    #         # Check if this is an entering detection
    #         entering, fraction = is_entering_store_percent(bbox, line_y=108, threshold=0.60)
            
    #         if entering:
    #             logger.info(f"Detection with bbox {bbox} is entering the store (fraction: {fraction}).")
    #             entering_detections.add(det_idx)
    #         elif bbox[1] > new_id_y_threshold and not startup_grace_period:  
    #             # y1 > threshold means it's below the threshold
    #             # Only apply this constraint if we're not in the startup grace period
    ...
    #     # Step 4: Process overlapping detections
    #     overlap_detections = set()
    #     matched_track_ids = {}  # {det_idx: track_id} for initially matched IDs
        
    #     # First, try to match all center detections as they must use existing IDs
    #     for det_idx in center_detections:
    #         if det_idx in assigned_detections:
    #             continue
    ...
    #         # For center detections, use a much higher threshold to force matches
    #         # We'll sort by best match later if there are conflicts
    #         relaxed_threshold = 0.9  # Very high threshold to ensure finding a match
            
    #         matched_id = None
    #         best_distance = float('inf')
            
    #         # Try all tracks and database entries
    #         for track in self.tracks:
    ...
    #         # If no active track matched, try database
    ...
    #                         distance = calculate_cosine_distance(detection.feature, db_feature)
    #                         if distance < best_distance:
    #                             best_distance = distance
    #                             matched_id = db_id
            
    #         if matched_id is not None:
    #             matched_track_ids[det_idx] = matched_id
    #             logger.info(f"Center detection {det_idx} matched with ID {matched_id} (distance: {best_distance}).")
                
    #             # Mark as overlapping for conflict resolution later
    #             if det_idx in overlap_groups and len(overlap_groups[det_idx]) > 0:
    #                 overlap_detections.add(det_idx)
    #         else:
    #             # This should never happen with relaxed threshold, but just in case
    #             logger.warning(f"CRITICAL: Center detection {det_idx} couldn't be matched to any ID.")
    #             matched_id = self._next_id
    #             matched_track_ids[det_idx] = matched_id
    #             self._next_id += 1
        
    #     # Now process remaining overlapping detections that aren't in center
    #     for det_idx in unmatched_detections:
    #         if det_idx in assigned_detections or det_idx in entering_detections or det_idx in center_detections:
    #             continue
                
    #         if det_idx in overlap_groups and len(overlap_groups[det_idx]) > 0:
    #             overlap_detections.add(det_idx)
                
    #             # Find potential match in global database
    #             detection = detections[det_idx]
    #             bbox = detection.to_tlbr()
    #             matched_id = self._match_with_global_database_all_tracks_considered(detection.feature, bbox)
                
    #             if matched_id is not None:
    #                 matched_track_ids[det_idx] = matched_id
    #                 logger.info(f"Matched overlapping detection {det_idx} with track_id {matched_id}")
        
    #     # Step 5: Identify ID conflicts among overlapping detections
    #     conflict_groups = {}  # {track_id: [det_idx1, det_idx2, ...]}
        
    #     for det_idx, track_id in matched_track_ids.items():
    #         if track_id in assigned_ids:
    #             # Conflict with already assigned IDs
    #             if track_id not in conflict_groups:
    #                 conflict_groups[track_id] = []
    #             conflict_groups[track_id].append(det_idx)
    #         elif track_id in [matched_track_ids.get(other_idx) for other_idx in overlap_detections if other_idx != det_idx]:
    #             # Conflict with other overlapping detections
    #             if track_id not in conflict_groups:
    #                 conflict_groups[track_id] = []
    #             conflict_groups[track_id].append(det_idx)
        
    #     # Step 6: Resolve ID conflicts using competition
    #     final_assignments = {}  # {det_idx: (track_id, is_new_id)}
        
    #     # First, resolve conflicts using competition
    #     for track_id, conflicting_detections in conflict_groups.items():
    #         # Calculate distances for all conflicting detections
    #         distances = {}
    #         for det_idx in conflicting_detections:
    #             detection = detections[det_idx]
    #             distance = self._calculate_distance_to_id(detection.feature, track_id)
    #             distances[det_idx] = distance
    #             logger.info(f"Conflict resolution: Detection {det_idx} has distance {distance} with track_id {track_id}")
            
    #         # Find the detection with the best match (lowest distance)
    #         best_detection = min(distances.items(), key=lambda x: x[1])[0]
    #         logger.info(f"Conflict resolved: Detection {best_detection} wins track_id {track_id}")
            
    #         # Assign this ID to the winning detection
    #         final_assignments[best_detection] = (track_id, False)
    #         assigned_detections.add(best_detection)
    #         assigned_ids.add(track_id)
            
    #         # Other conflicting detections will need new IDs or alternative matches
    #         for det_idx in conflicting_detections:
    #             if det_idx != best_detection:
    #                 # For center detections, must find alternative existing ID
    #                 if det_idx in center_detections:
    #                     alt_id = self._find_best_match_regardless_of_threshold(detection.feature, assigned_ids)
                        
    #                     if alt_id is not None:
    #                         logger.info(f"Center detection {det_idx} gets alternative ID {alt_id}")
    #                         final_assignments[det_idx] = (alt_id, False)
    #                     else:
    #                         # This is a fallback that should rarely happen - use oldest available ID
    ...
    #                 else:
    #                     # Regular detection, try to find alternative match
    #                     detection = detections[det_idx]
    #                     alt_id = self._find_next_best_match(detection.feature, detection.to_tlbr(), assigned_ids)
                        
    #                     if alt_id is not None:
    #                         logger.info(f"Found alternative match: detection {det_idx} gets track_id {alt_id}")
    #                         final_assignments[det_idx] = (alt_id, False)
    #                         assigned_detections.add(det_idx)
    #                         assigned_ids.add(alt_id)
    #                     else:
    #                         # New ID needed, but only if not in center or in startup grace period
    #                         if det_idx not in center_detections or startup_grace_period:
    #                             logger.info(f"No alternative match found: detection {det_idx} gets new track_id {self._next_id}")
    #                             final_assignments[det_idx] = (self._next_id, True)
    #                             assigned_detections.add(det_idx)
    #                             assigned_ids.add(self._next_id)
    #                             self._next_id += 1
    #                         else:
    #                             # Center detection must get existing ID
    #                             logger.warning(f"Center detection {det_idx} couldn't be assigned any existing ID.")
    #                             # As fallback, use the oldest ID in database
    #                             alt_id = min(self.global_database.keys()) if self.global_database else self._next_id
    #                             final_assignments[det_idx] = (alt_id, False)
    #                             assigned_detections.add(det_idx)
    #                             assigned_ids.add(alt_id)
        
    #     # Step 7: Handle non-conflicting overlapping detections
    #     for det_idx in overlap_detections:
    #         if det_idx in assigned_detections:
    #             continue
                
    #         if det_idx in matched_track_ids:
    #             track_id = matched_track_ids[det_idx]
    #             if track_id not in assigned_ids:
    #                 logger.info(f"Non-conflicting match: detection {det_idx} gets track_id {track_id}")
    #                 final_assignments[det_idx] = (track_id, False)
    #                 assigned_detections.add(det_idx)
    #                 assigned_ids.add(track_id)
    #             else:
    #                 # This shouldn't happen if conflict resolution worked, but just in case
    #                 # For center detections, must find alternative existing ID
    #                 if det_idx in center_detections:
    #                     detection = detections[det_idx]
    #                     alt_id = self._find_best_match_regardless_of_threshold(detection.feature, assigned_ids)
    ...
    #                     assigned_ids.add(alt_id)
    #                 else:
    #                     # Regular detection, can get new ID
    #                     logger.info(f"ID {track_id} already assigned. Giving detection {det_idx} new ID {self._next_id}")
    #                     final_assignments[det_idx] = (self._next_id, True)
    #                     assigned_detections.add(det_idx)
    #                     assigned_ids.add(self._next_id)
    #                     self._next_id += 1
    ...
    #                 # Center detection must get an existing ID, unless in startup grace period
    #                 detection = detections[det_idx]
    #                 alt_id = self._find_best_match_regardless_of_threshold(detection.feature, assigned_ids)
                    
    #                 if alt_id is not None:
    #                     logger.info(f"Center detection {det_idx} gets best available ID {alt_id}")
    #                     final_assignments[det_idx] = (alt_id, False)
    #                 else:
    #                     # Fallback to oldest ID
    #                     alt_id = min(self.global_database.keys()) if self.global_database else self._next_id
    ...
    #                 assigned_ids.add(alt_id)
    #             else:
    #                 # Non-center detection can get new ID (or we're in startup grace period)
    #                 logger.info(f"No match found for {'overlapping ' if det_idx in overlap_detections else ''}detection {det_idx}. Assigning new ID {self._next_id}")
    #                 final_assignments[det_idx] = (self._next_id, True)
    #                 assigned_detections.add(det_idx)
    #                 assigned_ids.add(self._next_id)
    #                 self._next_id += 1
        
    #     # Step 8: Handle entering detections
    #     for det_idx in entering_detections:
    #         # Always give entering detections new IDs
    #         logger.info(f"Entering detection {det_idx} gets new track_id {self._next_id}")
    #         final_assignments[det_idx] = (self._next_id, True)
    #         assigned_detections.add(det_idx)
    #         assigned_ids.add(self._next_id)
    #         self._next_id += 1
        
    #     # Step 9: Handle remaining non-overlapping detections
    #     remaining_dets = [det_idx for det_idx in unmatched_detections 
    #                     if det_idx not in assigned_detections 
    #                     and det_idx not in overlap_detections
    #                     and det_idx not in entering_detections
    #                     and det_idx not in center_detections]
        
    #     for det_idx in remaining_dets:
    #         detection = detections[det_idx]
    #         bbox = detection.to_tlbr()
            
    #         # Try to match with global database
    #         matched_id = self._match_with_global_database(detection.feature, bbox)
            
    #         if matched_id is not None and matched_id not in assigned_ids:
    #             logger.info(f"Matched remaining detection {det_idx} with track_id {matched_id}")
    #             final_assignments[det_idx] = (matched_id, False)
    #             assigned_detections.add(det_idx)
    #             assigned_ids.add(matched_id)
    #         else:
    #             # No match or ID already assigned
    #             # Check if this detection is above the threshold (can get new ID) or we're in startup grace period
    #             if bbox[1] <= new_id_y_threshold or startup_grace_period:
    #                 # Can get a new ID if above threshold or in startup grace period
    #                 logger.info(f"No matching ID for detection {det_idx}. Assigning new ID {self._next_id}")
    #                 if startup_grace_period and bbox[1] > new_id_y_threshold:
    ...
    #     # Step 10: Collect track modifications
    #     tracks_to_update = {}  # {track_id: (detection, is_new_track)}
    #     tracks_to_mark_missed = set()  # Set of track_ids to mark as missed
    ...
    #     # Keep track of which track IDs are already active
    #     active_track_ids = set(track.track_id for track in self.tracks if track.is_confirmed())
        
    #     # Process all final assignments
    #     for det_idx, (track_id, is_new_id) in final_assignments.items():
    #         detection = detections[det_idx]
    ...
    #         if existing_track is not None and not is_new_id:
    #             # Update existing track
    #             tracks_to_update[track_id] = (detection, False)
    #         else:
    #             # Create new track
    ...
    #     # Step 11: Apply all modifications to the track list
    #     # First, update existing tracks and mark tracks as missed
    #     for track in self.tracks:
    #         if track.track_id in tracks_to_update:
    #             detection, is_new = tracks_to_update[track.track_id]
    ...
    #     # Step 12: Remove deleted tracks
    #     self.tracks = [t for t in self.tracks if not t.is_deleted()]
        
    #     # Step 13: Update distance metric
    #     active_targets = [t.track_id for t in self.tracks if t.is_confirmed()]
    #     features, targets = [], []
    #     for track in self.tracks:
    #         if not track.is_confirmed():
    ...
    #     # Direct calculation using cosine or Euclidean distance
    #     feature_a = np.array([feature])[0]
    #     feature_b = np.array([track.features[-1]])[0]
        
    #     # Choose the appropriate distance metric
    #     if hasattr(self.metric, "_metric"):
    #         return self.metric._metric(feature_a, feature_b)
    #     else:
    #         # Fallback to cosine distance
    #         return 1.0 - np.dot(feature_a, feature_b) / (
    #             np.linalg.norm(feature_a) * np.linalg.norm(feature_b) + 1e-10
    #         )
    ...
    def _match_with_all_ids(self, detection_feature, detection_bbox, assigned_pairs=None, threshold_modifier=0.0):
        """
        Match a detection with ALL track IDs, including currently visible ones.
        This is specifically for handling overlapping detections where ID reassignment may be necessary.
        
        Args:
            detection_feature: Feature vector of the detection
            detection_bbox: Bounding box of the detection in tlbr format
            assigned_pairs: Dictionary of already assigned {detection_idx: track_id} pairs
            threshold_modifier: Adjustment to the matching threshold (can be positive or negative)
            
        Returns:
            tuple: (matched_track_id, min_distance, is_visible_track) 
                matched_track_id: Best matching track ID
                min_distance: Distance score of the match
                is_visible_track: Boolean indicating if the match is with a currently visible track
        """
        def is_inside_store_center(bbox, center_bbox=(0, 108, 1152, 800)):
        ...
        matched_track_id = None
        is_visible_match = False
        
        # Set base threshold based on location
        if is_inside_store_center(detection_bbox):
            base_threshold = 0.5  # Increased threshold for center area
        else:
            base_threshold = 0.4  # Default threshold
            
        # Apply threshold modifier
        threshold = base_threshold + threshold_modifier
        logger.info(f"Detection bbox {detection_bbox} using threshold {threshold} (with modifier {threshold_modifier})")

        # Get list of currently visible track IDs
        visible_track_ids = {
            track.track_id: track for track in self.tracks 
            if track.is_confirmed() and track.time_since_update == 0
        }
        
        # PART 1: Check matches with currently visible tracks
        for track_id, track in visible_track_ids.items():
            if track.features:
                valid_distances = []
                ...
                for track_feature in track.features:
                    ...
                    track_feature_arr = np.asarray(track_feature).flatten()
                    
                    # Calculate cosine distance
                    dot_product = np.dot(detection_feature_arr, track_feature_arr)
                    norm_detection = np.linalg.norm(detection_feature_arr)
                    norm_track = np.linalg.norm(track_feature_arr)
                    cosine_similarity = dot_product / (norm_detection * norm_track)
                    cosine_distance = 1 - cosine_similarity
                    
                    # Only consider distances below threshold
                    if cosine_distance < threshold:
                        valid_distances.append(cosine_distance)
                
                if valid_distances:
                    smallest_distance = min(valid_distances)
                    logger.info(f"Visible track ID {track_id} has valid distances: {valid_distances}. Smallest: {smallest_distance}")
                    
                    if smallest_distance < min_distance:
                        min_distance = smallest_distance
                        matched_track_id = track_id
                        is_visible_match = True
        
        # PART 2: Check matches with global database (tracks not currently visible)
        for track_id, data in self.global_database.items():
            # Skip if this track is currently visible (already checked above)
            ...
            if valid_distances:
                smallest_distance = min(valid_distances)
                logger.info(f"Database track ID {track_id} has valid distances: {valid_distances}. Smallest: {smallest_distance}")
        ...
        if matched_track_id is not None:
            logger.info(f"Best match is track_id {matched_track_id} with distance {min_distance} (visible: {is_visible_match})")
            return matched_track_id, min_distance, is_visible_match
        else:
            logger.info("No match found among all IDs")
            return None, None, False


    # def update(self, detections): #comprehensive
    #     """
    #     Enhanced update method with comprehensive ID matching for overlapping detections.
    #     """
    #     # Step 1: Identify potentially overlapping detections
    #     overlap_threshold = 0.25  # IoU threshold
    #     num_dets = len(detections)
    ...
    #     # Step 3: Process matches from the cascade
    #     # Keep track of what's been assigned
    #     assigned_det_to_track = {}  # {detection_idx: track_obj}
    #     assigned_track_to_det = {}  # {track_id: detection_idx}
        
    #     for track_idx, detection_idx in matches:
    #         track = self.tracks[track_idx]
    #         assigned_det_to_track[detection_idx] = track
    #         assigned_track_to_det[track.track_id] = detection_idx
        
    #     # Step 4: Identify overlapping detections (both matched and unmatched)
    #     all_overlap_detections = set()
    #     for det_idx in range(len(detections)):
    #         if det_idx in overlap_groups and len(overlap_groups[det_idx]) > 0:
    #             all_overlap_detections.add(det_idx)
        
    #     # Step 5: Process ALL overlapping detections with comprehensive matching
    #     # This is the key difference - we consider both matched and unmatched overlapping detections
    #     # but ONLY for detections that are part of overlapping groups
        
    #     # First, compute potential matches for all overlapping detections
    #     potential_matches = {}
    #     for det_idx in all_overlap_detections:
    #         detection = detections[det_idx]
    #         bbox = detection.to_tlbr()
    #         entering, fraction = is_entering_store_percent(bbox, line_y=108, threshold=0.60)
            
    #         if entering:
    #             # Entering detections always get new IDs
    #             potential_matches[det_idx] = {
    #                 "match_id": None,
    #                 "score": float('inf'),
    #                 "entering": True,
    ...
    #         # For all other overlapping detections, find best match among ALL IDs
    #         # We only use _match_with_all_ids for overlapping detections
    #         match_id, match_score, is_visible = self._match_with_all_ids(
    #             detection.feature, bbox,
    #             assigned_pairs=assigned_track_to_det
    #         )
            
    #         potential_matches[det_idx] = {
    #             "match_id": match_id,
    #             "score": match_score if match_score is not None else float('inf'),
    #             "entering": False,
    #             "is_visible_match": is_visible
    #         }
        
    #     # Sort overlapping detections by match quality (lower score is better)
    #     sorted_overlap_dets = sorted(
    #         [det_idx for det_idx in potential_matches.keys() 
    #         if potential_matches[det_idx]["match_id"] is not None],
    #         key=lambda det_idx: potential_matches[det_idx]["score"]
    #     )
        
    #     # Keep track of IDs that need reassignment due to better matches
    #     detections_needing_reassignment = {}
        
    #     # First pass: Process best matches that may require ID reassignment
    #     for det_idx in sorted_overlap_dets:
    #         match_info = potential_matches[det_idx]
    #         match_id = match_info["match_id"]
    #         is_visible = match_info["is_visible_match"]
            
    #         # Skip if this detection is marked as entering
    #         if match_info["entering"]:
    #             continue
            
    #         # If this is a visible track match and that ID is already assigned to a different detection
    #         if is_visible and match_id in assigned_track_to_det:
    #             current_det_idx = assigned_track_to_det[match_id]
                
    #             # If this detection has a better match score than the current assignment
    #             if current_det_idx != det_idx:
    #                 current_match_score = float('inf')
    #                 if current_det_idx in potential_matches:
    #                     current_match_score = potential_matches[current_det_idx]["score"]
                    
    #                 if match_info["score"] < current_match_score:
    #                     logger.info(f"Detection {det_idx} has a better match ({match_info['score']}) "
    #                                 f"with ID {match_id} than detection {current_det_idx} ({current_match_score})")
                        
    #                     # Mark the current detection for reassignment
    #                     detections_needing_reassignment[current_det_idx] = {
    #                         "original_id": match_id,
    #                         "new_id": None  # Will be determined later
    #                     }
                        
    #                     # Reassign the ID to this detection
    #                     assigned_track_to_det[match_id] = det_idx
    #                     assigned_det_to_track[det_idx] = None  # Placeholder, will be updated later
                        
    #                     logger.info(f"Reassigned ID {match_id} from detection {current_det_idx} to detection {det_idx}")
    #                 else:
    #                     # This detection needs to find another ID
    #                     logger.info(f"Detection {det_idx} has a worse match ({match_info['score']}) "
    #                                 f"with ID {match_id} than detection {current_det_idx} ({current_match_score})")
    #                     detections_needing_reassignment[det_idx] = {
    #                         "original_id": match_id,
    #                         "new_id": None  # Will be determined later
    #                     }
                
    #         # If this ID isn't assigned yet, assign it
    #         elif match_id not in assigned_track_to_det:
    #             assigned_track_to_det[match_id] = det_idx
    #             logger.info(f"Assigned ID {match_id} to detection {det_idx}")
        
    #     # Second pass: Find alternative IDs for detections that lost their best match
    #     # This includes both reassigned detections and those that had conflicts
    #     for det_idx in detections_needing_reassignment:
    #         detection = detections[det_idx]
    #         bbox = detection.to_tlbr()
            
    #         # Find next best match, excluding the original ID
    #         original_id = detections_needing_reassignment[det_idx]["original_id"]
    #         excluded_ids = set(assigned_track_to_det.keys())
            
    #         # Try with a slightly relaxed threshold for these "second choice" matches
    #         next_match_id, next_match_score, _ = self._match_with_all_ids(
    #             detection.feature, bbox,
    #             assigned_pairs=assigned_track_to_det,
    #             threshold_modifier=0.1  # Slightly relax the threshold
    #         )
            
    #         if next_match_id is not None and next_match_id not in assigned_track_to_det:
    #             # Found a good alternative match
    #             detections_needing_reassignment[det_idx]["new_id"] = next_match_id
    #             assigned_track_to_det[next_match_id] = det_idx
    #             logger.info(f"Found alternative ID {next_match_id} for detection {det_idx} "
    #                         f"(original was {original_id})")
    #         else:
    #             # No good alternative, will need a new ID
    #             detections_needing_reassignment[det_idx]["new_id"] = None
    #             logger.info(f"No good alternative ID for detection {det_idx}, will assign new ID")
        
    #     # Step 6: Create/update tracks based on all assignments
        
    #     # First, update tracks that were matched in the cascade and not reassigned
    #     for track_idx, det_idx in matches:
    #         track = self.tracks[track_idx]
    #         track_id = track.track_id
            
    #         # Skip if this track's ID was reassigned to another detection
    #         if track_id in assigned_track_to_det and assigned_track_to_det[track_id] != det_idx:
    #             logger.info(f"Skipping update for track {track_id} (detection {det_idx}) "
    #                         f"because it was reassigned to detection {assigned_track_to_det[track_id]}")
    #             continue
            
    #         # If this detection wasn't reassigned, update normally
    #         if det_idx not in detections_needing_reassignment:
    #             track.update(self.kf, detections[det_idx], self.global_database)
    #             logger.info(f"Normal update for track {track_id} with detection {det_idx}")
        
    #     # Next, handle all ID reassignments
    #     for det_idx, reassign_info in detections_needing_reassignment.items():
    #         detection = detections[det_idx]
    #         new_id = reassign_info["new_id"]
            
    #         # Initialize Kalman filter for this detection
    #         mean, covariance = self.kf.initiate(detection.to_xyah())
    #         class_name = detection.get_class()
            
    #         if new_id is not None:
    #             # Create track with the alternative ID found
    #             self.tracks.append(Track(
    ...
    #             # Copy features from global database if available
    #             if new_id in self.global_database:
    #                 self.tracks[-1].features = self.global_database[new_id]["features"]
    #                 logger.info(f"Created track with alternative ID {new_id} for detection {det_idx} "
    #                             f"with {len(self.tracks[-1].features)} features")
    #             else:
    #                 logger.info(f"Created track with alternative ID {new_id} for detection {det_idx} (no database features)")
    #         else:
    #             # Assign a new ID
    #             self.tracks.append(Track(
    #                 mean, covariance, self._next_id, self.n_init, self.max_age,
    #                 detection.feature, class_name
    #             ))
    #             logger.info(f"Assigned new ID {self._next_id} to detection {det_idx} that needed reassignment")
    #             self._next_id += 1
        
    #     # Finally, handle remaining detections that weren't part of the reassignment process
    #     remaining_detections = list(set(unmatched_detections) - set(detections_needing_reassignment.keys()))
        
    #     # Process all remaining unmatched detections
    #     for det_idx in remaining_detections:
    #         detection = detections[det_idx]
    #         bbox = detection.to_tlbr()
    #         entering, fraction = is_entering_store_percent(bbox, line_y=108, threshold=0.60)
            
    #         if entering:
    #             # Entering detection gets a new ID
    #             logger.info(f"Detection {det_idx} is entering the store (fraction: {fraction})")
    #             match_id = None
    #         else:
    #             # Use standard database matching for non-overlapping detections
    #             # We don't use _match_with_all_ids here since this detection is not overlapping
    #             match_id = self._match_with_global_database(detection.feature, bbox)
                
    #             # Skip if this ID is already assigned
    #             if match_id in assigned_track_to_det:
    #                 logger.info(f"ID {match_id} already assigned, finding next best match for detection {det_idx}")
    #                 match_id = self._find_next_best_match(detection.feature, bbox, set(assigned_track_to_det.keys()))
    ...
    #             # Create track with matched ID
    #             self.tracks.append(Track(
    #                 mean, covariance, match_id, self.n_init, self.max_age,
    #                 detection.feature, class_name
    #             ))
                
    #             # Copy features from global database
    #             if match_id in self.global_database:
    #                 self.tracks[-1].features = self.global_database[match_id]["features"]
    #                 logger.info(f"Matched unmatched detection {det_idx} with database ID {match_id} "
    #                             f"({len(self.tracks[-1].features)} features)")
    #             else:
    #                 logger.info(f"Matched unmatched detection {det_idx} with database ID {match_id} (no database features)")
    ...
    #                 detection.feature, class_name
    #             ))
    #             logger.info(f"Created new track with ID {self._next_id} for unmatched detection {det_idx}")
    #             self._next_id += 1
        
    #     # Step 7: Mark unmatched tracks as missed
    #     for track_idx in unmatched_tracks:
    #         self.tracks[track_idx].mark_missed()
        
    #     # Step 8: Remove deleted tracks and update distance metric (unchanged)
    #     self.tracks = [t for t in self.tracks if not t.is_deleted()]
    ...
    def _match_with_global_database_with_score(self, detection_feature, detection_bbox, center_bbox=(0, 108, 1152, 800)):
        """
        Match a detection with tracks in the global database.
        Returns the track_id with the smallest distance and the distance value.
        
        Args:
            detection_feature: Feature vector of the detection.
            detection_bbox: Bounding box of the detection in tlbr format.
            center_bbox: Definition of the store center area.
            
        Returns:
            tuple: (matched_track_id, min_distance) or (None, None) if no match found.
        """
        ...
        # Set threshold based on location
        if is_inside_store_center(detection_bbox, center_bbox):
            threshold = 0.5  # Increased threshold for center area
        ...
        for track_id, data in self.global_database.items():
            ...
            # Initialize a list to store valid distances for this track_id
            valid_distances = []
            ...
            for db_feature in data["features"]:
                ...
                # Only consider distances less than threshold
                if cosine_distance < threshold:
                    valid_distances.append(cosine_distance)
        ...
        if matched_track_id is not None:
            ...
            return matched_track_id, min_distance
        else:
            logger.info("No match found in global database.")
            return None, None
